...
class MoviesTopRankedRLM: Object {
    ...
    public func intFromResponse(response: Movies.Fetch.MovieModel) {
        ...
        for movie in response.movies! {
            let movieRLM = MoviesTopRankedRLM()
    ...
    public func findMovieByID(_ ID: String) -> Results<MoviesTopRankedRLM> {
        let realm = try! Realm()
        let predicate = NSPredicate(format: "id = %@", ID)
        return realm.objects(MoviesTopRankedRLM.self).filter(predicate)
    }
    ...
    public func getObjects() -> [Movies.Fetch.MovieModel.Movie] {
        ...
        for movies in realm.objects(MoviesTopRankedRLM.self) {
    ...
    public func howManyObjectsHave() -> Int {
        let realm = try! Realm()
        let howMany = realm.objects(MoviesTopRankedRLM.self)
