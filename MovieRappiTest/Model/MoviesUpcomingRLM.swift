//
//  MoviesUpcomingRLM.swift
//  MovieRappiTest
//
//  Created by Victor Manuel Lagunas Franco on 5/24/19.
//  Copyright © 2019 Victor Manuel Lagunas Franco. All rights reserved.
//

//
//  MoviesTopRankedRLM.swift
//  MovieRappiTest
//
//  Created by Victor Manuel Lagunas Franco on 5/24/19.
//  Copyright © 2019 Victor Manuel Lagunas Franco. All rights reserved.
...
class MoviesUpcomingRLM: Object {
    ...
    public func intFromResponse(response: Movies.Fetch.MovieModel) {
        ...
        for movie in response.movies! {
            let movieRLM = MoviesUpcomingRLM()
    ...
    public func findMovieByID(_ ID: String) -> Results<MoviesUpcomingRLM> {
        let realm = try! Realm()
        let predicate = NSPredicate(format: "id = %@", ID)
        return realm.objects(MoviesUpcomingRLM.self).filter(predicate)
    }
    ...
    public func getObjects() -> [Movies.Fetch.MovieModel.Movie] {
        ...
        for movies in realm.objects(MoviesUpcomingRLM.self) {
    ...
    public func howManyObjectsHave() -> Int {
        let realm = try! Realm()
        let howMany = realm.objects(MoviesUpcomingRLM.self)
